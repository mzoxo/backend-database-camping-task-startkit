...
from (
	...
	from "CREDIT_PURCHASE"
	where "CREDIT_PURCHASE".user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
	group by "CREDIT_PURCHASE".user_id) as cp
inner join (
	select 
	"COURSE_BOOKING".user_id,
	...
	from "COURSE_BOOKING"
	where "COURSE_BOOKING".user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') and status != '課程已取消'
	group by "COURSE_BOOKING".user_id) as cb
...
from "COURSE_BOOKING"
where 
	"COURSE_BOOKING".created_at >= '2024-11-01 00:00:00' and "COURSE_BOOKING".created_at <= '2024-11-30 23:59:59' and "COURSE_BOOKING".status != '課程已取消';